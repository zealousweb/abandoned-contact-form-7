...
if ( !class_exists( 'CF7AF_Admin_Filter' ) ) {
	...
	class CF7AF_Admin_Filter {
		...
		function __construct() {
			...
			add_filter( 'wp_insert_post_data',   				 array( $this, 'filter__post_data' ) );
		...
		*/
		 ...
		 *
		 * - Used to update post meta
		 *
		 * @method filter__post_data
		 *
		 * @param  array $data
		 *
		 * @return array
		 */
		function filter__post_data( $data){

			if ( CF7AF_POST_TYPE != $data['post_type'] ) {
				return $data;
			}

			if ( !isset( $_REQUEST['post_ID'] ) || !isset( $_REQUEST['cf7af_mail_status'] ) ) {
				return $data;
			}

			$post_id =  $_REQUEST['post_ID'];
			$cf7af_mail_status =  sanitize_text_field($_REQUEST['cf7af_mail_status']);
			update_post_meta( $post_id, 'cf7af_mail_status', $cf7af_mail_status );
